...
from sy_mutator import DartsMutator
from sy_mutator import RandomMutator
...
class DartsTrainer(Trainer):
    ...
    def __init__(self, model, loss, metrics,
        ...
        # super().__init__(model, mutator if mutator is not None else RandomMutator(model), # RandomMutator(model)
        super().__init__(model, mutator if mutator is not None else DartsMutator(model), # RandomMutator(model)
    ...
    def train_one_epoch(self, epoch):
        ...
        for step, ((trn_X, trn_y), (val_X, val_y)) in enumerate(zip(self.train_loader, self.valid_loader)):
            ...
            if self.unrolled:
                ...
                self._backward(val_X, val_y) # 验证集更新结构参数
            self.ctrl_optim.step()

            # phase 2: child network step
            self.optimizer.zero_grad()
            # print("->{}".format(step))
            # print("->{} {}".format(step, self.model))
            logits, loss = self._logits_and_loss(trn_X, trn_y) # 训练集更新权重参数
            # print("<-{}".format(step))
            # print("<-{} {}".format(step, self.model))
            # 权重在更新，但是不改变现有结构
            loss.backward()
    ...
    def _logits_and_loss(self, X, y):
        self.mutator.reset() # sample_search
        logits = self.model(X)
        # model_summary(self.model, X)
