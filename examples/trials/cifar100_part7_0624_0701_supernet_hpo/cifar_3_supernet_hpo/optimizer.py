...
def get_optim(args, model):
    if args.layerwise_lr:
        ...
        if len(args.lr_group)==16:

            lr_3 = args.lr_group[1:8]
            lr_5 = args.lr_group[1:8]
            lr_d3 = args.lr_group[8:15]
            lr_d5 = args.lr_group[8:15]

            lr_group = []  #
            lr_group.append(args.lr_group[0])  #
            for i in range(7):
                lr_group.append(lr_3[i])
                lr_group.append(lr_5[i])
                lr_group.append(lr_d3[i])
                lr_group.append(lr_d5[i])
            lr_group.append(args.lr_group[15])  #
        else:
