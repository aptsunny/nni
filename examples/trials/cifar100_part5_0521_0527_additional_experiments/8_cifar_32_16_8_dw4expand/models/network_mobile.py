...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, outplanes, shadow_bn, conv_first_dw = False, kernel=[3,3], activation=nn.ReLU):
    ...
    def __init__(self, shadow_bn, layers=12, classes=10, sample={}):
        ...
        for i in range(self.layers):
            if i in [0, 2]:  # layer1, layer3
                # print(sample[i])
                if i ==0:
                    self.Block.append(Bottleneck(channel[i], channel[i + 1], shadow_bn, conv_first_dw=False, kernel=sample[i]))
                elif i==2:
                    self.Block.append(
                        Bottleneck(channel[i], channel[i + 1], shadow_bn, conv_first_dw=True, kernel=sample[i]))
            else:
