...
def fast_9_lr_parameters_tail(model, lr_group, arch_search=None):
    rest_name = []
    figure_ = []
    for name, param in model.named_parameters():
        rest_name.append(param)
...
def prepare(args, epoch):
    ...
    net = SuperNetwork_3(shadow_bn=True, layers=3, classes=100, sample={0:[3,3],2:[3,5]})
    ...
    optimizer = torch.optim.SGD(fast_9_lr_parameters_tail(net, lr_group),
                                momentum=0.9,
                                weight_decay=5e-4)
