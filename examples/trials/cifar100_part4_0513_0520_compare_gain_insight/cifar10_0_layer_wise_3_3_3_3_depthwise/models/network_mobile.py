...
class Bottleneck(nn.Module):
    def __init__(self, inplanes, outplanes, shadow_bn, kernel=[3,3], activation=nn.ReLU):
        ...
        for t in self.expansion_rate:
            ...
            for j in self.kernel_list_0:
                ...
                self.conv_first.append(nn.Sequential(
                    # dw
                    nn.Conv2d(inplanes, outplanes, 3, 1, 1, groups=inplanes, bias=False),
            ...
            for j in self.kernel_list_1:
                ...
                # conv_list.append(self.dwconv(outplanes, outplanes))

                conv_list.append(nn.Sequential(
                    # dw
                    nn.Conv2d(outplanes, outplanes, 3, 1, 1, groups=outplanes, bias=False),
                    nn.BatchNorm2d(outplanes),
                    nn.ReLU(inplace=True),
                    # pw
                    nn.Conv2d(outplanes, outplanes, 1, 1, 0, bias=False),
                    nn.BatchNorm2d(outplanes),
                    nn.ReLU(inplace=True)
                ))
            ...
            for j in self.kernel_list_2:
                ...
                # conv_end.append(self.dwconv(outplanes, outplanes))
                conv_end.append(nn.Sequential(
                    # dw
                    nn.Conv2d(outplanes, outplanes, 3, 1, 1, groups=outplanes, bias=False),
                    nn.BatchNorm2d(outplanes),
    ...
    def dwconv(self, in_channels, out_channels, stride=1):
...
class SuperNetwork_3(nn.Module):
    def __init__(self, shadow_bn, layers=12, classes=10, sample={}):
        ...
        #     nn.ReLU(inplace=True)
        # )
        self.stem = self.dwconv(3, channel[0])
        ...
        for i in range(self.layers):
            if i in [0, 2]:  # layer1, layer3
                ...
                #
                self.Block.append(nn.Sequential(
                    # self.dwconv(channel[i], channel[i + 1]),
                    # dw
                    nn.Conv2d(channel[i], channel[i], 3, 1, 1, groups=channel[i], bias=False),
                    nn.BatchNorm2d(channel[i]),
                    nn.ReLU(inplace=True),
                    # pw
                    nn.Conv2d(channel[i], channel[i + 1], 1, 1, 0, bias=False),
                    nn.BatchNorm2d(channel[i + 1]),
                    nn.ReLU(inplace=True),
                    nn.MaxPool2d(2)
                    )
